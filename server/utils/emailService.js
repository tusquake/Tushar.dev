...
// Helper function to dispatch emails via Google Apps Script Web App
const sendViaGoogleScript = async ({ to, subject, html }) => {
  ...
  try {
    const response = await fetch(process.env.GOOGLE_SCRIPT_URL, {
      ...
      body: JSON.stringify({
        apiKey: process.env.GOOGLE_SCRIPT_API_KEY || 'MySuperSecretCode123',
        to,
        subject,
        html
      })
    ...
    console.error('Google script mail dispatch failed:', error);
...
const sendContactEmail = async ({ name, email, subject, message }) => {
  const subjectText = `Portfolio Contact: ${subject || 'New Message'} from ${name}`;
  const htmlContent = `
  ...
  if (process.env.GOOGLE_SCRIPT_URL) {
    ...
    console.warn('Google Script dispatch failed, falling back to SMTP...', res.error);
  }
...
const sendResetPasswordEmail = async ({ email, name, resetUrl }) => {
  const subjectText = 'Portfolio: Reset Password Request';
  const htmlContent = `
...
const sendWelcomeEmail = async ({ email, name }) => {
  const subjectText = 'Welcome to CodeForge! 🚀 Your Developer Adventure Begins Now';
  const htmlContent = `
    <!DOCTYPE html>
    <html>
    <head>
...
const sendSubscriptionEmail = async ({ email, name, tier, expiresAt }) => {
  ...
  const subjectText = `Your CodeForge ${currentTier.name} Subscription is Active! 🎉`;
  const htmlContent = `
    <!DOCTYPE html>
    <html>
    <head>
