...
const LatexResume = () => {
    ...
    const handleTemplateChange = (type) => {
        if (window.confirm('Switching templates will overwrite your current edits. Continue?')) {
    ...
    const handleReset = () => {
        if (window.confirm('Reset current editor code to the default template? All edits will be lost.')) {
    ...
    const handleImportFromBuilder = () => {
        ...
        if (!savedData) {
            alert('No resume data found in Builder. Please create a resume in the Builder first!');
            return;
        }
        ...
        try {
            ...
            alert('Failed to parse builder profile data.');
        }
    };
